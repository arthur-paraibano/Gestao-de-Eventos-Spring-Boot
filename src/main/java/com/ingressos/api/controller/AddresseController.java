...
import com.ingressos.api.service.AddresseService;
...
@RestController
@RequestMapping(value = "/addresse", produces = { "application/json" })
@Tag(name = "Addresse API", description = "Addresse Controller")
public class AddresseController {

    @Autowired
    private AddresseService service;
    @Autowired
    ...
    public ResponseEntity<List<AddresseModel>> findAll() {
        try {
            List<AddresseModel> EquipamentModels = service.findAll();
            for (AddresseModel uModel : EquipamentModels) {
                Integer id = uModel.getId();
                GeneralDto dtoId = new GeneralDto(id);
                uModel.add(linkTo(methodOn(AddresseController.class).getById(dtoId)).withSelfRel());
            }
    ...
    public ResponseEntity<AddresseModel> getById(@Valid @RequestBody GeneralDto id) {
        try {
            AddresseModel uModel = service.findById(id);
            uModel.add(linkTo(methodOn(AddresseController.class).findAll()).withRel("findAll"));
