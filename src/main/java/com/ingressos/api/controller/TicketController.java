...
@Tag(name = "Ticket API", description = "Ticket Controller")
public class TicketController {
    ...
    public ResponseEntity<List<TicketLotModel>> findAll() {
        try {
            ...
            for (TicketLotModel uModel : EquipamentModels) {
                ...
                uModel.add(linkTo(methodOn(TicketController.class).getById(dtoId)).withSelfRel());
            }
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successful operation", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = RestMessage.class)) }),
            @ApiResponse(responseCode = "204", description = "Ticket not found", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = RestMessage.class)) }),
            @ApiResponse(responseCode = "500", description = "Internal server error", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = RestMessage.class)) })
    })
    public ResponseEntity<TicketLotModel> getById(@Valid @RequestBody GeneralDto id) {
        try {
            ...
            uModel.add(linkTo(methodOn(TicketController.class).findAll()).withRel("findAll"));
