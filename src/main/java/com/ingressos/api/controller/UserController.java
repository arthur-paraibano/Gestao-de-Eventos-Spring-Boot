...
public class UserController {
    ...
    public ResponseEntity<List<UserModel>> findAll() {
        try {
            ...
            for (UserModel uModel : userModels) {
                ...
                uModel.add(linkTo(methodOn(UserController.class).getById(dtoId)).withSelfRel());
            }
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successful operation", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = RestMessage.class)) }),
            @ApiResponse(responseCode = "204", description = "User not found", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = RestMessage.class)) }),
            @ApiResponse(responseCode = "500", description = "Internal server error", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = RestMessage.class)) })
    })
    public ResponseEntity<UserModel> getById(@Valid @RequestBody GeneralDto id) {
        try {
